...
public class RedisStatBrowserTest extends BrowserTestTemplate {
	...
	@Autowired
	StatRepositoryAdapter statRepositoryAdapter;

	@Autowired
	RedisConnectionFactory redisConnectionFactory;

	@MockitoBean
	StatJpaRepository statJpaRepository; // JPA 미사용
	...
	void totalRankingNewUserWithoutJpa() throws Exception {
		...
		ResultActions result = mockMvc.perform(
			get("/stats/rankings").param("nickname", nickname)
		);
	...
	void totalRankingDeleteUserFallbackToJpa() throws Exception {
	    ...
	    // when
	    mockMvc.perform(delete("/user/me").session(session))
			.andExpect(status().isNoContent());
	...
	private void warmingRedisOneUser(User user) {
		StatWithNicknameAndUserId mockStat = new StatWithNicknameAndUserId(
			user.getId(), user.getNickname(), 10, 10, 100);
