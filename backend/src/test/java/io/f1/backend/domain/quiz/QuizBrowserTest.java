...
class QuizBrowserTest extends BrowserTestTemplate {
    ...
    void createTextQuiz() throws Exception {
        ...
        TextQuizCreateRequest request = new TextQuizCreateRequest("test", "test description", questions);
    ...
    void createImageQuiz() throws Exception {
        ...
        ImageQuizCreateRequest request = new ImageQuizCreateRequest("test", "test description", questions);
        MockPart requestPart = createJsonMockPart("request", request);
        
        // when
        MockMultipartHttpServletRequestBuilder mockMvcBuilder = multipart("/quizzes/image")
            .file(thumbnailFile)
            .part(requestPart);
    ...
    private MockPart createJsonMockPart(String requestPart, Object requestObject) throws Exception{
        MockPart part = new MockPart(requestPart, objectMapper.writeValueAsString(requestObject).getBytes(StandardCharsets.UTF_8));
    ...
    private void mockFileManager(MockedStatic<FileManager> mockFileManager) {
        mockFileManager.when(() -> FileManager.saveMultipartFile(any(MultipartFile.class), anyString()))
            .thenReturn("testpath");
        mockFileManager.when(() -> FileManager.getExtension(anyString()))
            .thenReturn("jpg");
    }
}
